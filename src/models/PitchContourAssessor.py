...
class PitchContourAssessor(nn.Module):
	...
	def __init__(self):
		...
		self.conv3 = nn.Conv1d(16, self.n_features, self.kernel_size, self.stride)
	...
	def forward(self, input):
		...
		Args:
			input: 			torch Variable (mini_batch_size x zero_pad_len), of input pitch contours, where
			...
			seq_lengths:	torch tensor (mini_batch_size x 1), containing lengths of each pitch contour
		"""
		...
		self.initHidden(mini_batch_size)
		lstm_out, self.hidden = self.lstm(conv3_out.transpose(1,2)) # transpose to ensure sequence length is dim 1 now
		
		# extract final output of the lstm layer (TODO: take into account length of individual pitch contour)
