...
filename = file_info + '_best'
if torch.cuda.is_available():
    perf_model.cuda()
    #perf_model.load_state_dict(torch.load('/Users/michaelfarren/Desktop/MusicPerfAssessment-master/src/runs/' + filename + '.pt'))
    perf_model.load_state_dict(torch.load('saved_runs/' + filename))

else:
    perf_model.load_state_dict(torch.load('saved_runs/' + filename + '.pt', map_location=lambda storage, loc: storage))
