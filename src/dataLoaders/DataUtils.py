...
class DataUtils():
	...
	def __init__(self, path_to_annotations, path_to_data, band, instrument):
		...
		Arg:
			...
			path_to_data:			string, full path to the folder containing the FBA audio files & pyin pitch tracks
	...
	def get_segment_info(self, year, segment, student_ids = []):
		...
		for student_id in student_ids:
			segment_file_path = annotations_folder + str(student_id) + '/' + str(student_id) + '_segment.txt'
	...
	def get_pitch_contours_segment(self, year, segment, student_ids = []):
		...
		idx = 0
		for student_id in student_ids:
			pyin_file_path = data_folder + str(student_id) + '/' + str(student_id) + '_pyin_pitchtrack.txt'
	...
	def get_perf_rating_segment(self, year, segment, student_ids = []):
		...
		for student_id in student_ids:
			ratings_file_path = annotations_folder + str(student_id) + '/' + str(student_id) + '_assessments.txt'
			...
			# convert to tuple and append
			perf_ratings.append((to_floats[2], to_floats[3], to_floats[4], to_floats[5]))
